...
function ProductListingContent() {
  ...
  const { products, currentPage, totalPages, setCurrentPage, getFilteredProducts, isLoading, error } = useProductStore();
  ...
  useEffect(() => {
    fetchAllProducts();
  }, [currentPage, selectedCategories, selectedSizes, selectedBrands, selectedColors, priceRange, sortBy, sortOrder, searchQuery]);
  ...
  const handleToggleFilter = (filterType: "categories" | "sizes" | "brands" | "colors", value: string) => {
    ...
    setterMap[filterType]((prev) => (prev.includes(value) ? prev.filter((v) => v !== value) : [...prev, value]));
  };
  ...
  const FilterSection = () => (
    <div className="space-y-8">
            ...
            brandCategory.brands.map((b) => (
              <div key={b.name} className="flex items-center">
                <Checkbox
                  ...
                  id={b.name}
                />
                <Label htmlFor={b.name} className="ml-2 text-sm text-gray-700">{b.name}</Label>
              </div>
            ))
          )}
        </div>
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {colors.map((color) => (
            <button
              key={color.name}
              className={`w-7 h-7 rounded-full ${color.class} ${selectedColors.includes(color.name) ? "ring-2 ring-offset-2 ring-gray-700" : "hover:ring-2 hover:ring-gray-300"} transition-all duration-200`}
              title={color.name}
      ...
      <div>
        <h3 className="mb-3 font-semibold text-lg text-gray-800">Price Range</h3>
        <Slider
          ...
          <div className="flex items-center gap-4">
            <Dialog>
              <DialogTrigger asChild>
                {/* Wrap multiple children inside a single element */}
            ...
            <Select value={`${sortBy}-${sortOrder}`} onValueChange={handleSortChange} name="sort">
        ...
        <div className="flex gap-10">
          ...
          <main className="flex-1">
                  ...
                  >
                    ...
                    <div className="p-4">
                      ...
                      <div className="flex items-center justify-between mt-3">
                        <p className="text-xl font-semibold text-gray-900">{product.price.toFixed(0)} ETB</p>
                        <div className="flex items-center gap-2">
                          {[...Array(5)].map((_, i) => (
                            <Star
                              key={i}
                              className={`h-5 w-5 ${
                                i < Math.round(product.rating || 0) ? "text-blue-500 fill-blue-500" : "text-gray-300"
                              }`}
                            />
                          ))}
            ...
            <div className="mt-12 flex justify-center items-center gap-2">
              <Button disabled={currentPage === 1} variant="outline" size="icon" onClick={() => handlePageChange(Math.max(1, currentPage - 1))}>
